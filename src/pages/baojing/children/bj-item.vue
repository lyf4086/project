<template>
  <div class="item-main" v-if="!!item.created">
    <div class="title" style="color:#ffffff">
      ...
      <div class="name">
        <p>{{item.policeuser.policeuser_name}}<i :title="item.policeuser.police_number" >{{item.policeuser.police_number}}</i></p>
    ...
    <div class="content">
      ...
      <div class="tex_wrap">
        ...
        <div class="list">
          <span>枪支类型：</span>
          <span>{{gunCode(item.gun.gtype)}}</span>
        </div>
        <div class="list">
          <span>描述：</span>
        ...
        <div class="list">
          <span>是否处理：</span>
          <span :class="chuli" ><i></i>{{text}}</span>
        </div>
      </div>
      <div class="sel" v-show="!item.desc" :class="{selected:item.checked}" @click="changeOne(item.index)"></div>
    </div>
  </div>
</template>
<style scoped>
