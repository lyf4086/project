      ...
      <div class="alert">
        <div class="content" v-show="upDataShow">
          ...
          <div class="floor">
            ...
            <div class="r">
              <span>警员：</span>
              <el-select v-model="jingyuanSel" placeholder="请选择">
                <el-option
                  v-for="item in jingyuanList"
                  :key="item.id"
                  :label="item.uname"
                  :value="item.id"
                ></el-option>
          ...
          <div class="floor">
            <div class="l">
              ...
              <el-select v-model="qiangzhiSel" placeholder="请选择">
                <el-option
                  v-for="item in qiangzhiList"
                  :key="item.id"
                  :label="`枪支编号：${item.gun_code}`"
                  :value="item.id"
                ></el-option>
              </el-select>
            </div>
          ...
          <div class="over" @click="yanzheng" v-show="!jingyuanSel|| !qiangzhiSel || !shipingName"></div>
          <el-upload
  ...
  data() {
    return {
      URL:
        ...
        "/weixin/project/index.php?m=Home&c=Uploads&a=upload",
      dataList: [],
      currentNodeKey: "",
      pageTotal: 0,
      active_page: 1,
  ...
  methods: {
    ...
    getDataList(jigou_id, active_p = 1) {
      ...
      this.$axios({
        url:
          this.$store.state.baseURL +
          "/weixin/project/index.php?m=home&c=Video&a=index",
        method: "POST",
        changeOrigin: true,
        data: params
      })
        .then(data => {
          ...
          if (data.data.code == 200) {
            this.jingyuanList=data.data.pol
            this.qiangzhiList=data.data.guns
            this.jingyuanStr = `<option value="" disabled selected hidden>请选择人员</option>`;
            let str1 = data.data.pol
              .map(item => {
    ...
    deleteMessage(vidArr) {
      ...
      this.$axios({
        url:
          this.$store.state.baseURL +
          "/weixin/project/index.php?m=home&c=Video&a=dele",
        method: "POST",
        changeOrigin: true,
        data: params
      })
    ...
    downLoad(id) {
      ...
      this.$axios({
        url:
          this.$store.state.baseURL +
          "/weixin/project/index.php?m=Home&c=Down&a=download",
        method: "POST",
        changeOrigin: true,
        data: params,
        responseType: "blob"
