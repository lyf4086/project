<template>
    <div class="guiji">
      ...
      <div class="wrap">
        <div id="map"></div>
        <div class="left_mes"
          v-show="filterMessage.uname"
        >
          <div class="main_mes" >
            <i class="view" @click="binaZuBack" v-show="!oneAlarmPersonList.length||oldOrNew=='new'"
              :class="{'left_view':!checked_person_show}" v-if="!oneAlarmPersonList.length"
            ></i>
            <i class="view" v-if="oneAlarmPersonList.length" @click="alarmBoxShowOrHide"
            ...
            <div class="btn_wrap" :no_click="noClick">
              ...
              <div class="right_btn">
                <div class="no_click" v-if="noClick">
          ...
          <div class="item_list" v-show="checked_person_show"  v-if="!oneAlarmPersonList.length">
            ...
            <div class="item_wrap_scroll">
              <div class="item" v-for="item,index in checkedPersonArr" :key="index">
                ...
                <p class="btns">
                  <span title="个人详情" @click="spanToXiangQing(item)"><i class="el-icon-view"></i></span>
          ...
          <div class="one_alarm_person_list" v-if="oneAlarmPersonList.length && oneAlarmPersonListBox" >
      ...
      <div class="sel_wrap">
        <div class="sel_item">
          机构：
          <select v-html="jigouOptionStr" @change="jigouSelChange">

          </select>
        </div>
        <div class="sel_item" v-show="gunAndJingyuanOptionStr">
          <span>警员及枪支：</span>
          <select v-html="gunAndJingyuanOptionStr" @change="jingyuanSelChange">
 ...
 <!-- 所有报警区域列表 -->
      <div class="alarm_list" v-show="role_id!=3">
        <select name="" id="" class="sel" v-html="allAlarmSelectStr" ref="alarmSelect" @change="showOne">
...
export default {
  ...
  methods: {
    ...
    mapInit(){
      let that=this
       BM.Config.HTTP_URL = 'http://192.168.1.110:9000';
       if(this.map){
         this.map.remove()
       }
      var map = BM.map('map', 'bigemap.awiawk58', { center: [39.90,116.32], zoom: 1, zoomControl: true });
      //  map.fitBounds([[39.44145965576172, 115.41685485839844], [41.05923080444336, 117.50813293457031]]);
      // let marker=BM.marker([39.90923,116.397428],{
      //   title:'我是测试图标'
      // }).addTo(map)
    ...
    setMarker(arr){
      ...
      // let marker =BM.marker(arr,{
      //   title:'我是测试图标'
      // }).addTo(map)
      // marker.bindPopup('我是测试4<br>444444444444<br>4444444444444弹窗');
    },
    setPolyGon(arr){
       let that=this
      let BM=this.BM
      let map=this.map
      return BM.polygon(arr, {color: 'blue'}).addTo(map);
    },
    newShuaXinMap(){
