     ...
     function creatTreeStr(obj){
          ...
          if(obj.length>0){
              for (let i = 0; i < obj.length; i++) {
                  ...
                  str +=`<option value="${obj[i].id}" > ${obj[i].mechanism_name}</option>`;
...
function searchHistory(IMEI,stime,etime,ps=999){//......获取历史轨迹函数
  var key=this.$store.state.key
  var objs = {"IMEI":IMEI,"stime":stime,"etime":etime,"ps":ps};
  var sign =  this.$methods.mkSign(objs,key);
  var token=this.$gscookie.getCookie('gun')
  var params = new URLSearchParams();
  params.append('IMEI', objs.IMEI);
...
function getPersonAndGunStr(id){
  var key=this.$store.state.key
  var objs = {p:1,ps:10,mechanism_id:id};
  var sign =  this.$methods.mkSign(objs,key);
  var token=this.$gscookie.getCookie('gun')
  var params = new URLSearchParams();
  params.append('p', objs.p);
  ...
  this.$axios({
      ...
      function optionStr(arr){
        ...
        arr.forEach(e=>{
          ...
          str+=`<option value="${e.IMEI}|${e.policeuser.img}"  >枪支编号：${e.gun_code} , 所属警员：${e.policeuser_name ?e.policeuser_name:'暂未绑定人员'}</option>`
        })
        return str
      }
  }).catch((error)=>{
...
function setDian(e){
  ...
  let polyLine=this.BM.polyline(that.polyLineArr, {color: 'blue'}).addTo(map);
...
function confirmSetArea(){
  ...
  let that=this
  let BM=this.BM
  // let arr=this.markerArr.map(e=>e.G.position)
  // console.log(this.polyLineArr)
  let polygon=BM.polygon(that.polyLineArr, {color: 'blue'}).addTo(map)
...
function shezhiquyu(gun_ids,pointsArr,policeuser_id,stime,etime,text,IMEIStr){
  ...
  var objs = {
     "gun_ids":idsStr,"points":pointsStr,"policeuser_id":policeuser_id,
      "stime":newStime,"etime":newEtime,"type":typeNumber,"mechanism_id":this.selectedPerson.mechanism_id,
      "area_alarm_name":text,"IMEI":IMEIStr
    };
...
function getOneAlarmArea(id){//.....获取一个报警区域
  ...
  this.$axios({
    ...
    if(data.data.code==200){
      // console.log('748',data.data.data)
      let id=this.$refs.alarmSelect.value
      this.newShuaXinMap()
      this.showOneAreaAllMarker(data.data.data.list)
      this.showOneAlarmPolygon(data.data.data.list.points,id)
...
function showOneAlarmPolygon(arr,id){
  ...
  if(this.shangyigequyu){//先把刚才查看的区域清空
   ...
   let gangcaide=this.setPolyGon(arr2)
...
function getNewPosition(){
  ...
  this.$axios({
    ...
    if(data.data.code==200){
      console.log('三分钟后zuo 运动',this.markerArr,this.moveingPersonList,data.data.data.list)
