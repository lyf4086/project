<template>
  <div id="home-child">
    <div class="top-sels">
      ...
      <div class="btn-wrap" @click="subCheckDate">
          确定
      </div>
    </div>
    <div class="bottom-chart-two-wrap">
        ...
        <div id="item1_echart_right">rrr</div>
    </div>
    <div class="bottom-chart-only" >
        <div id="item1_echart_only">bottom-chart-one-wrap</div>
   ...
   methods: {
    ...
    next1() {
      if (this.dbtimestart && this.dbtimeend) {
        if (
          new Date(this.dbtimestart).getTime() >
          new Date(this.dbtimeend).getTime()
        ) {
          this.$message({
            type: "error",
            message: "请重新选择时间"
    ...
    getDuiBi(mid, ip_id, tid, time) {
      ...
      this.$axios({
        url:
          ...
          "/weixin/project/index.php?m=Home&c=Index&a=alarm_anal",
        method: "POST",
        changeOrigin: true,
        data: params
      })
    ...
    subCheckDate() {
      ...
      let [timeStart, timeEnd] = this.datePicker;
    ...
    chartNew2() {
      let that = this;
      let box = document.getElementById("item1_echart_only");
      let EchartNew = this.$echarts.init(box);
      function rgb() {
        //rgb颜色随机
